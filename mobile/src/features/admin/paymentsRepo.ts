...
export async function fetchPendingPaymentSubmissions(propertyIds: string[]): Promise<PaymentSubmissionRow[]> {
...
export async function createSignedProofUrl(objectPath: string, expiresIn = 7200): Promise<string | null> {
...
export async function approvePaymentSubmission(
  ...
  const { error: e2 } = await supabase.from("properties").update({ is_delinquent: false }).eq("id", propertyId);
  if (e2) return { error: e2.message };

  return {};
}

export async function rejectPaymentSubmission(submissionId: string, adminNotes: string): Promise<{ error?: string }> {
